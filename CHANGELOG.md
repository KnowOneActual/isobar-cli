...
### Fixed
- **API Request Reliability** — Implemented an automatic retry mechanism with exponential backoff for API client calls (geocoding, weather, and AQI) to handle transient network issues and timeouts. Adjusted timeout values to prevent long-running hangs.

## [1.3.6] - 2026-06-09

### Fixed
- **CI/CD** — Upgraded `pip` in the security audit workflow to resolve `PYSEC-2026-196` vulnerability warning.

## [1.3.5] - 2026-06-04

### Fixed
- **Linting** — Fixed import sorting formatting in test files.

## [1.3.4] - 2026-06-04

### Fixed
- **API Error Handling** — Fixed an issue where network/API errors incorrectly reported that the city was not found. Now they are correctly propagated and printed to the user, and the CLI exits with code 1.
...
### Fixed
- **Version Consistency** — Fixed `__version__` attribute in `__init__.py` to match pyproject.toml
